...
export class FunctionDevComponent extends ViewInfoComponent implements AfterViewInit, AfterContentInit, OnDestroy {
    ...
    constructor(private _broadcastService: BroadcastService,
        ...
        super('function-dev', _functionAppService, _broadcastService, () => _globalStateService.setBusyState());
    ...
    setup(): Subscription {
        return this.viewInfoEvents
            .switchMap(functionView => {
                ...
                this.viewInfo = functionView;
                this.context = functionView.context;
                ...
                return Observable.zip(
                    this._functionAppService.getAuthSettings(functionView.context),
                    functionView.functionInfo.isSuccessful
                        ? this._functionAppService.getFunctionErrors(functionView.context, functionView.functionInfo.result)
                        : Observable.of(functionView.functionInfo),
            ...
            .subscribe(tuple => {
                if (tuple[0].isSuccessful && tuple[1].isSuccessful && tuple[2].functionInfo.isSuccessful) {
                    const functionInfo = tuple[2].functionInfo.result;
                    this.content = '';
                    this.eventGridSubscribeUrl =
                        `${this.context.mainSiteUrl.toLowerCase()}/admin/extensions/EventGridExtensionConfig?functionName=${functionInfo.name}&code=${tuple[3].result}`;;
