...
export class FunctionAppService {
    ...
    getFunction(context: FunctionAppContext, name: string): Result<FunctionInfo> {
        return this.getFunctions(context)
            .map(r => {
                if (r.isSuccessful) {
                    const functionInfo = r.result.find(f => f.name.toLocaleLowerCase() === name.toLocaleLowerCase());
                    return functionInfo
                        ? Object.assign(r, { result: functionInfo })
                        : {
                            result: null,
                            isSuccessful: false,
                            error: {
                    ...
                    return r as any as FunctionAppHttpResult<FunctionInfo>;
                }
            });
    }
    ...
    getFunctionAppAzureAppSettings(context: FunctionAppContext) {
        return this.azure.executeWithConditions([], context, t =>
            this._cacheService.post(`${context.site.id}/config/appsettings/list`, true, this.jsonHeaders(t))
    ...
    runHttpFunction(context: FunctionAppContext, functionInfo: FunctionInfo, url: string, model: HttpRunModel): Result<RunFunctionResult> {
        return this.runtime.executeWithConditions([], context, token => {
            ...
            const headers = this.headers(token, ['Content-Type', contentType]);
